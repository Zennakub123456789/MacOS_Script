...
-- Override default settings if MacUI.Settings exists
if MacUI.Settings and MacUI.Settings.Override then
    MacUI.Settings.Override("Notifications", "enabled", true)
    MacUI.Settings.Override("Notifications", "duration", 3)
end
...
local ConfigFolder = "PvB_Configs"
local ConfigFile = "PvB_Settings.json"
...
local PvBConfig = {
    ...
    -- Auto Open Eggs
    SelectedEggs = {"Godly Lucky Egg"},
    AutoOpenEgg = false,
    
    -- Auto Favorite
    BrainrotRaritySelection = {},
    AutoFavoriteBrainrotToggle = false,
    PlantRaritySelection = {},
    AutoFavoritePlantToggle = false,
    SharedMutationSelection = {},
    AutoFavoriteBrainrotByMutation = false,
    AutoFavoritePlantByMutation = false,
    ...
    AutoSell = false,
    AutoSellFull = false,
    PlantSellDelaySlider = 1,
    AutoSellPlants = false,
    AutoSellPlantsFull = false,
    AutoSellAllFull = false,
    
    -- Event
    AutoTurnIn = false,
    AutoResetRequest = false,
...
local function SavePvBConfig()
    if not isfolder(ConfigFolder) then
        makefolder(ConfigFolder)
    end
    
    local filePath = ConfigFolder .. "/" .. ConfigFile
    local jsonString = HttpService:JSONEncode(PvBConfig)
    writefile(filePath, jsonString)
    print("[PvB Config] Saved to:", filePath)
end

local function LoadPvBConfig()
    local filePath = ConfigFolder .. "/" .. ConfigFile
    ...
    if isfile(filePath) then
        local success, result = pcall(function()
            ...
            print("[PvB Config] Loaded successfully")
            return true
        end)
        ...
        if success then
            ...
            warn("[PvB Config] Failed to load:", result)
        end
    end
...
local Window = MacUI:Window({
    ...
    Theme = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("General", "theme")) or "Dark",
    Icon = iconPath,
    LoadingTitle = "MacUI",
    LoadingSubtitle = "Loading...",
    ToggleUIKeybind = "K",
    ShowText = "Menu",
    NotifyFromBottom = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("Notifications", "fromBottom")) or true,
    ConfigurationSaving = {
        Enabled = true,
        FileName = "PvB_Config"
    },
    ...
    KeySettings = {
        ...
        SaveKey = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("KeySystem", "saveKey")) or true
    }
})
...
getgenv().AntiAFK = PvBConfig.AntiAFK or false

local AntiAFKToggle = MainTab:Toggle({
    Title = "Anti AFK",
    Default = PvBConfig.AntiAFK or false,
    Flag = "AntiAFK",
    Callback = function(value)
        getgenv().AntiAFK = value
        PvBConfig.AntiAFK = value
...
getgenv().TeleportToBrainrot = PvBConfig.TeleportToBrainrot or false
local farmPart = nil
...
local AutoTeleportToggle = AutoTab:Toggle({
    ...
    Default = PvBConfig.TeleportToBrainrot or false,
    Flag = "TeleportToBrainrot",
    Callback = function(value)
        getgenv().TeleportToBrainrot = value
        PvBConfig.TeleportToBrainrot = value
...
local sharedMutationOptions = {"Radioactive", "Shiny", "Rainbow", "Reversed", "Inverted", "Corrupted", "Glitched"}
...
local SharedMutationDropdown = AutoTab:Dropdown({
    ...
    Callback = function(selected)
        getgenv().SharedSelectedMutations = selected
    end
})

local selectedMutationsBrainrot = {}
local selectedMutationsPlant = {}
local isFavoriteMutationBrainrotLoopRunning = false
local isFavoriteMutationPlantLoopRunning = false

local FavoriteBrainrotByMutationToggle = AutoTab:Toggle({
    Title = "Auto Favorite Brainrot By Mutation",
    Default = false,
    Flag = "AutoFavoriteBrainrotByMutation",
    Callback = function(value)
        if value then
            if isFavoriteMutationBrainrotLoopRunning then return end
            isFavoriteMutationBrainrotLoopRunning = true
            selectedMutationsBrainrot = getgenv().SharedSelectedMutations or {}
            MacUI:Notify({ Title = "Started", Content = "Start Auto Favorite Brainrots By Mutation", Duration = 3 })

            task.spawn(function()
                while isFavoriteMutationBrainrotLoopRunning do
                    local localPlayer = Players.LocalPlayer
                    if not localPlayer or not localPlayer.Character then break end
                    ...
                    for _, tool in ipairs(backpack:GetChildren()) do
                        local success, err = pcall(function()
                            if tool and tool:IsA("Tool") then
                                ...
                                local uuidValue = tool:GetAttribute("ID")
                                if uuidValue and #selectedMutationsBrainrot > 0 then
                                    local nestedModel = tool:FindFirstChild(brainrotValue)
                                    if nestedModel then
                                        local mutationValue = nestedModel:GetAttribute("Mutation")
                                        
                                        if mutationValue and table_contains(selectedMutationsBrainrot, mutationValue) then
                                            local args = { [1] = uuidValue }
                                            ReplicatedStorage.Remotes.FavoriteItem:FireServer(unpack(args))
                                            MacUI:Notify({ Title = "Auto Favorite", Content = "Favorite: " .. tool.Name, Duration = 3 })
                                        end
            ...
            if isFavoriteMutationBrainrotLoopRunning then
                isFavoriteMutationBrainrotLoopRunning = false
                MacUI:Notify({ Title = "Stopped", Content = "Stop Auto Favorite Brainrots By Mutation", Duration = 4 })
            end
        end
    end
})

local FavoritePlantByMutationToggle = AutoTab:Toggle({
    Title = "Auto Favorite Plants By Mutation",
    Default = false,
    Flag = "AutoFavoritePlantByMutation",
    Callback = function(value)
        if value then
            if isFavoriteMutationPlantLoopRunning then return end
            isFavoriteMutationPlantLoopRunning = true
            selectedMutationsPlant = getgenv().SharedSelectedMutations or {}
            MacUI:Notify({ Title = "Started", Content = "Start Auto Favorite Plants By Mutation", Duration = 3 })

            task.spawn(function()
                while isFavoriteMutationPlantLoopRunning do
                    local localPlayer = Players.LocalPlayer
                    if not localPlayer or not localPlayer.Character then break end
                    ...
                    for _, tool in ipairs(backpack:GetChildren()) do
                        local success, err = pcall(function()
                            if tool and tool:IsA("Tool") then
                                ...
                                local uuidValue = tool:GetAttribute("ID")
                                if uuidValue and #selectedMutationsPlant > 0 then
                                    local nestedModel = tool:FindFirstChild(plantValue)
                                    if nestedModel then
                                        local mutationValue = nestedModel:GetAttribute("Mutation")
                                        
                                        if mutationValue and table_contains(selectedMutationsPlant, mutationValue) then
                                            local args = { [1] = uuidValue }
                                            ReplicatedStorage.Remotes.FavoriteItem:FireServer(unpack(args))
                                            MacUI:Notify({ Title = "Auto Favorite", Content = "Favorite: " .. tool.Name, Duration = 3 })
                                        end
            ...
            if isFavoriteMutationPlantLoopRunning then
                isFavoriteMutationPlantLoopRunning = false
                MacUI:Notify({ Title = "Stopped", Content = "Stop Auto Favorite Plants By Mutation", Duration = 4 })
            end
        end
    end
})

local ShopTab = Window:Tab("Shop", "rbxassetid://10698878025")
...
local SeedDropdown = ShopTab:Dropdown({
    ...
    Flag = "SeedSelection",
    Callback = function(selected)
        getgenv().SelectedSeeds = selected
    end
})
            ...
            task.spawn(function()
                while getgenv().AutoBuyAllSeeds do
                    for _, seed in ipairs(AllSeeds) do
                        if not getgenv().AutoBuyAllSeeds then break end
                        buySeedRemote:FireServer(seed)
                        task.wait(0.5)
                    end
                end
...
local buyGearRemote = ReplicatedStorage.Remotes.BuyItem
...
local SaveConfigButton = SettingTab:Button({
    ...
    Callback = function()
        ...
        -- Save to Window Config
        if Window.SaveConfig then
            Window:SaveConfig()
            savedCount = savedCount + 1
        end
        
        -- Save to MacUI Settings
        if MacUI.Settings and MacUI.Settings.Save then
            MacUI.Settings.Save()
            savedCount = savedCount + 1
        end
...
local ResetConfigButton = SettingTab:Button({
    ...
    Callback = function()
        if Window.ResetConfig then
            ...
            MacUI:Notify({
                Title = "Reset Failed",
                Content = "Reset function not available",
                Duration = 3
            })
        end
    end
...
SettingTab:Section("MacUI Settings (Auto-Save)")

-- Theme Dropdown with MacUI.Settings
local ThemeDropdown = SettingTab:Dropdown({
    Title = "UI Theme",
    Options = {"Default", "Dark", "Ocean"},
    Default = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("General", "theme")) or "Dark",
    Flag = "UITheme",
    Callback = function(value)
        if MacUI.Settings and MacUI.Settings.Set then
            MacUI.Settings.Set("General", "theme", value)
            MacUI:Notify({
                Title = "Theme Changed",
                Content = "Theme: " .. value .. " (Restart to apply)",
                Duration = 3
            })
        end
    end
...
local NotifDurationSlider = SettingTab:Slider({
    Title = "Notification Duration",
    Min = 1,
    Max = 15,
    Default = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("Notifications", "duration")) or 3,
    Suffix = "s",
    Rounding = 0,
    Flag = "NotifDuration",
    Callback = function(value)
        if MacUI.Settings and MacUI.Settings.Set then
            MacUI.Settings.Set("Notifications", "duration", value)
        end
    end
})

-- Notifications From Bottom Toggle
local NotifFromBottomToggle = SettingTab:Toggle({
    Title = "Notifications From Bottom",
    Default = (MacUI.Settings and MacUI.Settings.Get and MacUI.Settings.Get("Notifications", "fromBottom")) or false,
    Flag = "NotifFromBottom",
    Callback = function(value)
        if MacUI.Settings and MacUI.Settings.Set then
            MacUI.Settings.Set("Notifications", "fromBottom", value)
            MacUI:Notify({
                Title = "Position Changed",
                Content = "Restart script to apply changes",
                Duration = 3
            })
        end
    end
...
local languageScripts = {
    ["English"] = function()
        ...
        FavoriteBrainrotByMutationToggle:SetTitle("Auto Favorite Brainrot By Mutation")
        FavoritePlantByMutationToggle:SetTitle("Auto Favorite Plants By Mutation")
    ...
    ["ภาษาไทย"] = function()
        ...
        FavoriteBrainrotByMutationToggle:SetTitle("ล็อคหัวใจ Brainrot อัตโนมัติ [ตามสถานะที่เลือก]")
        FavoritePlantByMutationToggle:SetTitle("ล็อคหัวใจ Plants อัตโนมัติ [ตามสถานะที่เลือก]")
        ...
        AutoTurnInToggle:SetTitle("เปิดอัตโนมัติ")
        AutoResetRequestToggle:SetTitle("รีเซ็ตคุกอัตโนมัติ")
        HideNotificationsToggle:SetTitle("ซ่อนการแจ้งเตือน")
        LowGraphicsToggle:SetTitle("กราฟิกต่ำ")
        LanguageDropdown:SetTitle("เลือกภาษา")
        ApplyButton:SetTitle("ยืนยัน")
        ApplyButton:SetDesc("ใช้ภาษาที่เลือก")
    end
}

ApplyButton.Callback = function()
    local scriptFunc = languageScripts[selectedLanguage]
    if scriptFunc then
        scriptFunc()
    end
end
